    ...
    <>
      <header className="sticky top-0 z-10 flex items-center justify-between gap-6 border-b border-[rgba(31,57,42,0.12)] bg-[rgba(247,245,239,0.84)] px-[clamp(20px,4vw,56px)] py-5 backdrop-blur-[18px]">
        <div className="flex items-center gap-4">
          ...
          <a
            className="inline-flex min-h-10 items-center justify-center rounded-lg border border-transparent px-4 text-sm font-semibold text-[#3f5045]"
      ...
      <main className="px-[clamp(20px,4vw,56px)] pb-10 pt-[clamp(42px,7vw,96px)]">
        <section className="grid items-center gap-[clamp(28px,5vw,72px)] lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.86fr)]">
          <div className="max-w-[760px]">
            ...
            <div
              className="mt-7 flex flex-wrap gap-2 text-sm text-[#54665a]"
              aria-label="Platform capabilities"
            >
              <span className="rounded-full border border-[rgba(31,57,42,0.14)] bg-white/55 px-3 py-[7px]">
                Clerk organizations
              </span>
              <span className="rounded-full border border-[rgba(31,57,42,0.14)] bg-white/55 px-3 py-[7px]">
                TanStack Start SSR
          ...
          <aside
            className="overflow-hidden rounded-lg border border-[rgba(31,57,42,0.16)] bg-[#102017] shadow-[0_24px_80px_rgba(18,35,25,0.24)]"
          ...
          >
            <div className="flex justify-between gap-3 border-b border-white/10 p-[18px] text-[#cad9cf]">
              <div>
                ...
                <p className="mt-1.5 text-[13px] text-[#96a99d]">
                  tenant: memo-demo / query: renewal risk
                </p>
              </div>
              <span className="self-start rounded-full bg-[rgba(66,185,112,0.18)] px-[10px] py-[6px] text-xs font-bold text-[#d7ffe2]">
                Live
              </span>
            </div>
            <div className="grid gap-3 p-[18px]">
              {memories.map((memory) => (
                <article
                  ...
                  key={memory.agent}
                >
                  <span className="text-xs font-bold uppercase tracking-wide text-[#8fb29d]">
                    {memory.agent}
                  </span>
                  <p className="m-0 leading-[1.5] text-[#eef7f0]">
                    {memory.text}
                  </p>
                  <span className="w-fit rounded-full bg-[rgba(246,221,155,0.1)] px-[9px] py-[5px] text-xs text-[#f6dd9b]">
                    {memory.score}
                  </span>
                </article>
              ))}
